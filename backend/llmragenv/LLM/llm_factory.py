...
from llmragenv.LLM.client_error import (
    ClientUrlFormatError,
    ClientAPIUnsupportedError,
    ClientError,
)
    ...
    def get_client(self) -> LLMBase:
        ...
        if self.llmbackend == "openai":
            if self.model_name in LLMProvider["zhipu"]:
                ...
                raise ClientAPIUnsupportedError(
                    f"No client API adapted for model: {self.model_name}"
                )
