...
class RetrieverGraph(object):
    ...
    def extract_keyword(self, question, max_keywords=5):
        prompt = keyword_extract_prompt.format(
            question=question, max_keywords=max_keywords
        )
        print(prompt)
        ...
        capitalized_keywords = [
            keyword.capitalize() for keyword in capitalized_keywords
        ]
    ...
    def retrieve_2hop(self, question, pruning=True, build_node=False):
        ...
        pruning_knowledge_sequence = semantic_pruning(
            question=question, knowledge_sequence=all_knowledge_sequence
        )
    ...
    def retrieve_2hop_with_keywords(
        self, question, keywords=[], pruning=None, build_node=False
    ):
        self.pruning = pruning
        ...
        if self.pruning:
            pruning_knowledge_sequence, pruning_knowledge_dict = self.postprocess(
                question, knowledge_sequence
            )

            if build_node:
                self.nodes = self.graph_database.build_nodes(
                    pruning_knowledge_sequence, pruning_knowledge_dict
                )
        else:
            pruning_knowledge_sequence = knowledge_sequence
            if build_node:
                self.nodes = self.graph_database.build_nodes(
                    knowledge_sequence, rel_map
                )
    ...
    def postprocess(self, question, knowledge_sequence):
        ...
        sorted_all_rel_scores = self.semantic_pruning_triplets(
            question, kg_triplets, rel_embeddings=embeddings, topk=self.pruning
        )
