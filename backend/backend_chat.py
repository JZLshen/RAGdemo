...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--llm",
        type=str,
        help="llm env (e.g., qwen0.5b, llama2:7b, llama2:13b, llama2:70b)",
        default="llama2:70b",
    )
    parser.add_argument(
        "--func",
        type=str,
        help="function (e.g., Without RAG, Graph RAG, Vector RAG, Union RAG)",
        default="Graph RAG",
    )
    parser.add_argument(
        "--graphdb",
        type=str,
        help="graph database baskend (e.g., neo4j, ) ",
        default="nebulagraph",
    )
    parser.add_argument(
        "--vectordb",
        type=str,
        help="vector database baskend (e.g., MilvusDB, ) ",
        default="MilvusDB",
    )
    parser.add_argument(
        "--space_name",
        type=str,
        help="space_name baskend (e.g., rgb, multihop) ",
        default="rgb",
    )
    parser.add_argument(
        "--llmbackend", type=str, help="openai or llama_index", default="llama_index"
    )
